"""
Phasic Policy Gradient Implementation
original paper: https://arxiv.org/abs/2009.04416
original implementation: https://github.com/openai/phasic-policy-gradient
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--beta-clone", type=float, default=1.0, help="the behavior cloning coefficient"
    )
    parser.add_argument(
        "--n-aux-minibatch",
        type=int,
...
class ProbsVisualizationWrapper(gym.Wrapper):
    ...
    def render(self, mode="human"):
        if mode == "rgb_array":
            ...
            fig, ax = plt.subplots(
                figsize=(self.image_shape[1] / 100, self.image_shape[0] / 100),
...
def make_env(gym_id, seed, idx):
    def thunk():
        ...
        env = wrap_pytorch(
            wrap_deepmind(env, clip_rewards=True, frame_stack=True, scale=False,)
        )
        env.seed(seed)
    ...
    for policy_update in range(1, args.n_iteration + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                ...
                if args.capture_video:
                    probs_list = np.array(
                        Categorical(logits=agent.actor(agent.network(obs[step])))
                        .probs[0:1]
                        .tolist()
                    )
        ...
        for i_epoch_pi in range(args.e_policy):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogproba, entropy = agent.get_action(
                    b_obs[minibatch_ind], b_actions.long()[minibatch_ind]
                )
    ...
    for auxiliary_update in range(1, args.e_auxiliary + 1):
        ...
        for start in range(0, args.aux_batch_size, args.aux_minibatch_size):
            ...
            kl_loss = td.kl_divergence(
                agent.get_pi(m_aux_obs), old_agent.get_pi(m_aux_obs)
            ).mean()
